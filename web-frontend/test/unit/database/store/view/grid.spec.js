...
describe('Grid view store', () => {
  ...
  test('UPDATE_GROUP_BY_METADATA_COUNT skips when group-by field is missing', () => {
    const state = Object.assign(gridStore.state(), {
      activeGroupBys: [
        { id: 1, field: 1, order: 'ASC' },
        { id: 2, field: 2, order: 'ASC' },
      ],
      groupByMetadata: {
        field_1: [
          { field_1: 'a', count: 2 },
        ...
        field_2: [
          { field_1: 'a', field_2: 1, count: 2 },
          { field_1: 'b', field_2: 1, count: 1 },
        ],
      },
    })
    ...
    const originalMetadata = JSON.parse(
      JSON.stringify(store.state.grid.groupByMetadata)
    )

    // Only field 1 is provided, field 2 is "hidden" / missing.
    expect(() => {
      store.commit('grid/UPDATE_GROUP_BY_METADATA_COUNT', {
        fields: [{ id: 1, name: 'Test 1', type: 'text', primary: true }],
        registry: testApp._app.$registry,
        row: { id: 1, field_1: 'a', field_2: 1 },
        increase: true,
        decrease: false,
      })
    }).not.toThrow()

    // The first group-by (field 1) is valid so its count is updated.
    // The second group-by (field 2) is skipped because field 2 is missing,
    // making that level of the hierarchy unresolvable.
    expect(store.state.grid.groupByMetadata).toEqual({
      field_1: [
        { field_1: 'a', count: 3 },
        { field_1: 'b', count: 1 },
