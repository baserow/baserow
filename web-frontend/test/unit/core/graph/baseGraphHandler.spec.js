...
// Corrupted-graph resilience: a point must never reference itself via next or
// children, but when it happens (real customer graphs), every traversal must
// terminate instead of hanging the page, and remove() must not splice the
// removed point back in as its own successor.
describe('BaseGraphHandler corrupted graph (self-references and cycles)', () => {
  test('getChildren terminates on a self-referencing next', () => {
    const h = make(
      { 0: 1, 1: { children: { '': [2] } }, 2: { next: { '': [2] } } },
      pm(1, 2)
    )
    const children = h.getChildren(pt(1), { followChains: true })
    expect(children.map((p) => p.id)).toEqual([2])
  })

  test('getChildren terminates on a chain looping back to an earlier point', () => {
    const h = make(
      {
        0: 1,
        1: { children: { '': [2] } },
  ...
  test('getChildren skips a container listed as its own child', () => {
    const h = make({ 0: 1, 1: { children: { '': [1] } } }, pm(1))
    expect(h.getChildren(pt(1), { followChains: true })).toEqual([])
    expect(h.getChildren(pt(1))).toEqual([])
  })

  test('getLastPosition terminates on a looping root chain', () => {
    const h = make(
      { 0: 1, 1: { next: { '': [2] } }, 2: { next: { '': [1] } } },
      pm(1, 2)
    )
    const [ref, position, output] = h.getLastPosition()
    expect(ref.id).toBe(2)
    expect([position, output]).toEqual(['south', ''])
  })

  test('getDescendantIds terminates on self-referencing next and child', () => {
    const h = make(
      {
        0: 1,
        1: { children: { '': [2] } },
    ...
    expect(h.getDescendantIds(1).map(Number)).toEqual([2, 3])
  })

  test('getPreviousPositions terminates on a cycle', () => {
    const h = make(
      { 0: 1, 1: { next: { '': [2] } }, 2: { next: { '': [2] } } },
      pm(1, 2)
    )
    const positions = h.getPreviousPositions(pt(2))
    expect(Array.isArray(positions)).toBe(true)
  })

  test('remove does not splice a self-referencing point back in as its own successor', () => {
    const h = make(
      { 0: 1, 1: { children: { '': [2] } }, 2: { next: { '': [2] } } },
      pm(1, 2)
    )
    h.remove(pt(2))
    expect(h.graph).toEqual({ 0: 1, 1: { children: { '': [] } } })
  })

  test('remove of the container of a self-referencing child terminates', () => {
    const h = make(
      {
        0: 3,
        3: { next: { '': [1] } },
    ...
    h.remove(pt(1))
    expect(h.graph).toEqual({ 0: 3, 3: { next: { '': [4] } }, 4: {} })
  })
})
