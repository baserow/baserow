<template>
  ...
  >
    <div class="notification-panel__notification-content-title">
      <i18n-t keypath="whiteboardCommentReplyNotification.title" tag="span">
        <template #sender>
          <strong v-if="sender">{{ sender }}</strong>
          <strong v-else>
            <s>{{ $t('whiteboardCommentReplyNotification.deletedUser') }}</s>
          </strong>
        </template>
        <template #whiteboard>
...
export default {
  name: 'WhiteboardCommentReplyNotification',
