<template>
  <div class="automation-app">
    <AutomationHeader :automation="automation" @read-only-toggled="handleReadOnlyToggle" />
  ...
  methods: {
    ...
    async handleAddNode({ type, previousNodeId }) {
      try {
        ...
        const currentNodesWithNew = this.$store.getters[
          'automationWorkflowNode/getAll'
        ]
        ...
        if (previousNodeId === null) {
          ...
          if (insertAfterIndex !== -1) {
            ...
            console.warn(
              `previousNodeId '${previousNodeId}'not found in existingIds (string array): [${existingIds.join(', ')}]. Appending ${newNodeIdString}.`
            )
