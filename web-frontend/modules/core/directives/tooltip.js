...
export default {
  ...
  initialize(el, binding) {
    ...
    // Add a flag to track if event listeners have been added
    if (!el._tooltipEventsAdded) {
      ...
      el.tooltipMouseEnterEvent = (event) => {
        ...
        if (el.tooltipOptions.contentClasses) {
          contentClasses = contentClasses.concat(
            el.tooltipOptions.contentClasses
          )
        }
      ...
      el.tooltipClose = () => {
        ...
        if (el.tooltipElement) {
          ...
          // Reset the content events flag when tooltip is closed
      ...
      // Add the main event listeners only once
