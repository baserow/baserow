...
export const actions = {
  ...
  updateWidget({ getters, commit }, { widgetId, values }) {
    return new Promise((resolve, reject) => {
      // TODO: use passed original values as these values
      // are the same
      const originalValuesCopy = JSON.parse(
        JSON.stringify(getters.getWidgetById(widgetId))
      )
      ...
      debouncedWidgetUpdate = debounce(async () => {
        try {
          ...
          commit('UPDATE_WIDGET', { widgetId, originalValuesCopy })
          reject(error)
        }
      }, 1000)
  ...
  async fetchInitial({ commit, dispatch }, dashboardId) {
    ...
    // TODO: reuse fetchNewDataSources action
  ...
  async deleteWidget({ commit }, widgetId) {
    ...
    // TODO: also clean up the data source from the store
  },
}
