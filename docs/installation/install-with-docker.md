...
4. A button click whose actions reach outside Baserow runs as a job on the
   `button_dispatch` queue, so a slow endpoint holds a Celery worker rather
   than a web worker. The default worker consumes `button_dispatch` and
   `automation_workflow` together with the `celery` queue that carries realtime
   updates. On a worker with a single process, a click waiting on a slow
   endpoint therefore delays realtime updates and automations until it
   finishes. Running more than one worker process (`BASEROW_AMOUNT_OF_WORKERS`)
   or a dedicated worker for `button_dispatch` avoids that.
    1. If you run the workers with your own `celery worker -Q ...` command,
       include `automation_workflow` and `button_dispatch`, or those clicks stay
       pending until the job cleanup fails them.
