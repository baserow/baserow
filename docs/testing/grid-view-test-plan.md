...
are parametrized to run in both flat and expanded Sections group-by views.
Columns-specific coverage is listed in section 9.6. Dedicated group-by rows above
cover creating inside a Sections group and changing a grouped value.
...
These simple-field filter cases run in both flat and expanded Sections group-by
views.
...
These simple-field sort cases run in both flat and expanded Sections group-by
views.
...
These add-in-group scenarios exercise the per-leaf add-row lines in Sections.
...
These saved filter cases run in both flat and expanded Sections group-by views.
...
These saved sort cases run in both flat and expanded Sections group-by views.
...
### 7.1.1 Sections views load expanded and collapse or expand all groups

- A grouped view using Sections initially renders its group banners expanded with
  their rows visible.
...
### 7.1.2 Collapse and expand a group in Sections
...
### 7.2.1 Adding a group-by in Sections refreshes with one metadata and one row request

- A Sections view already grouped by one field renders its group banners with
  counters.
...
### 7.3.1 Sections group headers show per-column aggregation summaries
...
### 7.3.2 Hovering a Sections group header to pick or change an aggregation
...
### 7.3.3 Live Sections group aggregation updates on row changes

- In Sections, editing a cell updates that row's group value (and its ancestor
...
### 9.1.2 Row coloring with Sections group-by

- Grid opens with the saved group-by and Sections layout applied.
...
- The group layout control uses the user-facing labels "Sections" and "Columns";
  their stored values are `section` and `column`.
...
### 9.6.2 Second level adds a column; Sections restores the banners
...
- Choosing "Sections" sends one view PATCH, restores the banners and removes every
  span.
...
- Switching from collapsed Sections to Columns expands the rendered hierarchy
  without overwriting the saved Sections collapse state.
...
- Fewer than 100 left rows, right rows and spans are mounted at both the start and
  ...
  below 250 combined.
- Switching back to Sections restores the collapsed state; switching to Columns
...
- TODO:
    ...
    - In Sections, collapsed group headers are never drop targets; unloaded row
...
- 7.x: Remaining Sections-specific standalone behavior not yet automated — each
  ...
  unit tests, not e2e); Sections group headers draw a vertical cell separator at
  ...
  multi-level Sections collapse/expand, counts, and add-row; lazy-loading rows
