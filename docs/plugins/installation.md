...
it, the new container is created from the `baserow/baserow:2.3.3` image which does not
...
Baserow also provides `baserow/backend:2.3.3` and `baserow/web-frontend:2.3.3` images
...
```
docker run --rm baserow/backend:2.3.3 install-plugin ... 
docker run -e BASEROW_PLUGIN_GIT_REPOS=https://example.com/example/plugin1.git,https://example.com/example/plugin2.git --rm baserow/backend:2.3.3
```
...
3. `docker run --rm -v baserow_data:/baserow/data baserow:2.3.3 uninstall-plugin plugin_name`
...
6. Rebuild your image - `docker build -t my-customized-baserow:2.3.3 .`
