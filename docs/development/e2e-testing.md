...
The e2e stack intentionally uses `3030`/`8030` by default so it does not
collide with the default dev stack (`3000`/`8000`) or the documented alternate
dev stacks (`3010`/`8010` and `3020`/`8020`).
...
# Access http://localhost:3030 in your browser
```
...
|----------|-------------|---------|
| `E2E_FRONTEND_PORT` | Host port published by `just e2e up` for the frontend | `3030` |
| `E2E_BACKEND_PORT` | Host port published by `just e2e up` for the backend API | `8030` |
| `E2E_FRONTEND_COOKIE_PREFIX` | Cookie prefix used by the e2e frontend to avoid same-host auth cookie collisions | `baserow_e2e_` |
| `PUBLIC_WEB_FRONTEND_URL` | Frontend URL for tests | `http://localhost:3030` |
| `PUBLIC_BACKEND_URL` | Backend API URL for tests | `http://localhost:8030` |
