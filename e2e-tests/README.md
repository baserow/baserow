...
| `E2E_SLACK_BOT_TOKEN` | `./run-e2e-tests-locally.sh` and `yarn run test` | A Slack bot token (`xoxb-...`) that may post to `E2E_SLACK_CHANNEL` (default `social`) in a scratch workspace. Only the one test in `button_field_slack.spec.ts` that really sends needs it; without it that test is skipped, since the stack has no way to stub slack.com. |
