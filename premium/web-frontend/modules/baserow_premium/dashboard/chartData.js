...
function getAggregationTitle($registry, aggregationType) {
  return $registry.get('groupedAggregation', aggregationType).getName()
}
...
function getResultPropertyName({ dataSource, $registry, propertyName }) {
  return getServiceType({ dataSource, $registry }).getResultPropertyName(
    dataSource,
    propertyName
  )
}
