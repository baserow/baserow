...
from baserow_enterprise.ai_assistant.types import (
    AiErrorMessage,
    AiMessage,
    ChatTitleMessage,
    HumanMessage,
...
def test_cannot_list_ai_assistant_chats_without_valid_workspace(
    api_client, enterprise_data_fixture
):
    _, token = enterprise_data_fixture.create_user_and_token()

    rsp = api_client.get(
        reverse("api:enterprise:ai_assistant:list"),  # missing workspace_id
        format="json",
    ...
    rsp = api_client.get(
        reverse("api:enterprise:ai_assistant:list")
    ...
    rsp = api_client.get(
        reverse("api:enterprise:ai_assistant:list") + f"?workspace_id={workspace.id}",
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert rsp.status_code == 400
...
@pytest.mark.django_db
@override_settings(DEBUG=True)
def test_cannot_list_ai_assistant_chats_without_license(
    api_client, enterprise_data_fixture
):
...
@pytest.mark.django_db
@override_settings(DEBUG=True)
def test_list_ai_assistant_chats(api_client, enterprise_data_fixture):
    user, token = enterprise_data_fixture.create_user_and_token()
    workspace = enterprise_data_fixture.create_workspace(user=user)
    ...
    chats_count = 10
    chats = [
        AiAssistantChat(workspace=workspace, user=user, title=f"Chat {i}")
        for i in range(chats_count)
    ]
    with freeze_time("2024-01-14 12:00:00"):
        AiAssistantChat.objects.bulk_create(chats)
    ...
    for i in range(chats_count):
        ...
        assert chat == {
            ...
            "title": f"Chat {i}",
            "status": AiAssistantChat.Status.IDLE,
...
def test_cannot_send_message_without_valid_workspace(
    ...
    rsp = api_client.post(
        reverse(
            "api:enterprise:ai_assistant:chat_messages",
    ...
    rsp = api_client.post(
        reverse(
            "api:enterprise:ai_assistant:chat_messages", kwargs={"chat_uuid": chat_uuid}
        ),
        data={
            "content": "Hello AI",
...
@pytest.mark.django_db
@override_settings(DEBUG=True)
@patch("baserow_enterprise.api.ai_assistant.views.AiAssistantHandler")
def test_send_message_creates_chat_if_not_exists(
    ...
    # Mock chat creation
    mock_chat = MagicMock(spec=AiAssistantChat)
...
def test_cannot_get_messages_without_valid_chat(api_client, enterprise_data_fixture):
    ...
    assert rsp.status_code == 404
    assert rsp.json()["error"] == "ERROR_AI_ASSISTANT_CHAT_DOES_NOT_EXIST"
