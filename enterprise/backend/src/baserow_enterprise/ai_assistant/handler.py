...
from baserow_enterprise.ai_assistant.assistant import Assistant
...
class AiAssistantHandler:
    def get_chat(self, user: AbstractUser, chat_uid: str | UUID) -> AiAssistantChat:
        """
        ...
        :raises AiAssistantChatDoesNotExist: If the chat does not exist.
        """

        try:
            return AiAssistantChat.objects.select_related(
                "workspace", "user__profile"
            ).get(user=user, uuid=chat_uid)
        except AiAssistantChat.DoesNotExist:
            raise AiAssistantChatDoesNotExist(
    ...
    def get_or_create_chat(
        ...
        try:
            ...
            created = False
        except AiAssistantChatDoesNotExist:
    ...
    ) -> list[AiAssistantChat]:
        """
        List all AI assistant chats for the user in the specified workspace.
        """

        return AiAssistantChat.objects.filter(
            workspace_id=workspace_id, user=user
        ).order_by("-updated_on", "id")

    def get_chat_messages(self, chat: AiAssistantChat) -> list[BaseMessage]:
        """
    ...
    def get_assistant(
        self, chat: AiAssistantChat, new_message: HumanMessage | None = None
    ) -> Assistant:
        """
