...
from baserow_enterprise.ai_assistant.graph.base import AssistantGraph, Node
...
class Assistant:
    def __init__(self, chat: AiAssistantChat, new_message: HumanMessage | None = None):
        self.chat = chat
        self.user = chat.user
    ...
    async def _lock_chat(self):
        ...
        try:
            self.chat.status = AiAssistantChat.Status.IN_PROGRESS
            await self.chat.asave(update_fields=["status", "updated_on"])
            yield
        finally:
            self.chat.status = AiAssistantChat.Status.IDLE
            await self.chat.asave(update_fields=["status", "updated_on"])
