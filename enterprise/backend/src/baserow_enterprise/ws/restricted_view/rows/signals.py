...
from baserow_enterprise.views.handler import EnterpriseViewHandler
...
@baserow_trace(tracer)
def restricted_view_rows_created(
    sender,
    rows,
    before,
    user,
    ...
    **kwargs,
    ...
    row_checker = EnterpriseViewHandler().get_restricted_views_row_checker(
...
@baserow_trace(tracer)
def restricted_view_before_rows_delete(sender, rows, user, table, model, **kwargs):
    row_checker = EnterpriseViewHandler().get_restricted_views_row_checker(
        table, model, only_include_views_which_want_realtime_events=True
    )
    return {
        "deleted_rows_restricted_views": (
            row_checker.get_filtered_views_where_rows_are_visible(rows)
        ),
...
@baserow_trace(tracer)
def restricted_view_rows_deleted(
    sender, rows, user, table, model, before_return, send_realtime_update=True, **kwargs
):
    if not send_realtime_update:
        return

    restricted_views = dict(before_return)[restricted_view_before_rows_delete][
        "deleted_rows_restricted_views"
    ]
    serialized_deleted_rows = dict(before_return)[restricted_view_before_rows_delete][
        "deleted_rows"
    ]
    transaction.on_commit(
        lambda: _send_rows_deleted_event_to_views(
            serialized_deleted_rows, restricted_views
        )
    )
...
@baserow_trace(tracer)
def restricted_view_before_rows_update(
    ...
    return {
        "old_rows_restricted_views": row_checker.get_filtered_views_where_rows_are_visible(
            rows
        ),
...
@baserow_trace(tracer)
def restricted_view_rows_updated(
    sender,
    rows,
    user,
    table,
    ...
    **kwargs,
    ...
    before_return_dict = dict(before_return)[restricted_view_before_rows_update]
    ...
    old_row_restricted_views: List[FilteredViewRows] = before_return_dict[
        "old_rows_restricted_views"
    ]
    existing_checker = before_return_dict["caching_row_checker"]
    restricted_view_rows: List[
        FilteredViewRows
    ] = existing_checker.get_filtered_views_where_rows_are_visible(rows)

    view_id_to_updated_restricted_view_rows = {
        view.view.id: view for view in restricted_view_rows
    }

    # When a row is updated from the point of view of a restricted view it might not
    # always result in a `rows_updated` event. For example if a row was previously
    # not visible in the restricted view due to its filters, but the row update makes it
    # now match the filters we want to send a `rows_created` event to that views page
    # as the clients won't know anything about the row and hence a `rows_updated`
    # event makes no sense for them.
    restricted_views_where_rows_were_created: List[FilteredViewRows] = []
    restricted_views_where_rows_were_updated: List[FilteredViewRows] = []
    restricted_views_where_rows_were_deleted: List[FilteredViewRows] = []

    for old_restricted_view_rows in old_row_restricted_views:
        (old_row_view, old_visible_ids) = old_restricted_view_rows

        updated_restricted_view_rows = view_id_to_updated_restricted_view_rows.pop(
            old_row_view.id, None
        )

        if updated_restricted_view_rows is None:
            restricted_views_where_rows_were_deleted.append(
                FilteredViewRows(old_row_view, None)
            )
        else:
            new_visible_ids = updated_restricted_view_rows.allowed_row_ids

            if (
                old_visible_ids == FilteredViewRows.ALL_ROWS_ALLOWED
                and new_visible_ids == FilteredViewRows.ALL_ROWS_ALLOWED
            ):
                restricted_views_where_rows_were_updated.append(
            ...
            if len(created_ids) > 0:
                restricted_views_where_rows_were_created.append(
    ...
    # Any remaining views in the updated_rows_restricted_views dict are views which
    # previously didn't show the old row, but now show the new row, so we want created.
    restricted_views_where_rows_were_created = (
        restricted_views_where_rows_were_created
        + list(view_id_to_updated_restricted_view_rows.values())
    )

    @baserow_trace(tracer)
    def _send_created_updated_deleted_row_signals_to_views():
        _send_rows_deleted_event_to_views(
            serialized_old_rows, restricted_views_where_rows_were_deleted
        )
        _send_rows_created_event_to_views(
            serialized_updated_rows,
            before=None,
            restricted_views=restricted_views_where_rows_were_created,
        )
