...
case "$1" in
    ...
    celery-worker)
      if [[ -n "${BASEROW_RUN_MINIMAL}" && $BASEROW_AMOUNT_OF_WORKERS == "1" ]]; then
        ...
        start_celery_worker -Q celery,export,automation_workflow,button_dispatch -n default-worker@%h "${@:2}"
      else
        export OTEL_SERVICE_NAME="celery-worker"
        start_celery_worker -Q celery,automation_workflow,button_dispatch -n default-worker@%h "${@:2}"
      fi
    ;;
