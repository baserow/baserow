...
@pytest.mark.django_db
def test_prepare_value_for_db_validates_external_image_protocols(data_fixture):
    _, _, field, field_type = _rich_text_field(data_fixture)
    user_file = data_fixture.create_user_file(original_name="a.png", is_image=True)
    value = (
...
@pytest.mark.django_db
def test_import_serialized_value_validates_external_image_protocols(data_fixture):
