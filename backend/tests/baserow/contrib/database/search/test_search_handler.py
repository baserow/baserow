...
def test_create_tsvector_columns(data_fixture):
    ...
    table = data_fixture.create_database_table(
        user, force_add_tsvectors=False, search_data_state=SearchTableState.DISABLED
    )
    ...
    table = SearchHandlerCompat.sync_tsvector_columns(table)
    for field in table.field_set.all():
...
def test_get_fields_missing_search_index(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(
        user, search_data_state=SearchTableState.DISABLED
    )
    model = table.get_model()
...
def test_updating_link_row_field_so_it_moves_between_tables_syncs_properly(
    ...
    table_c = data_fixture.create_database_table(
        user, database=database, search_data_state=SearchTableState.DISABLED
    )
    ...
    sh = SearchHandlerCompat()
    sh.update_tsvector_columns(table_c, False)
    sh.update_tsvector_columns(table_b, False)
    sh.update_tsvector_columns(table_a, False)
...
def test_updating_link_row_field_so_it_no_longer_has_related_field_syncs_tsvs(
    ...
    table_a, table_b, link_field = data_fixture.create_two_linked_tables(
        user=user,
        database=database,
        table_kwargs=dict(search_data_state=SearchTableState.DISABLED),
    )
...
@pytest.mark.django_db
@patch(
    "baserow.contrib.database.search.handler.SearchHandlerCompat.update_tsvector_columns"
)
def test_update_tsvector_columns_locked_without_cache_lock(mocked_update, data_fixture):
    table = data_fixture.create_database_table(
        search_data_state=SearchTableState.DISABLED
    )
...
def test_split_update_into_chunks_by_ranges(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(
        user=user, search_data_state=SearchTableState.DISABLED
    )
    ...
    SearchHandlerCompat().update_tsvector_columns(table, False)
...
def test_split_update_into_chunks_until_all_background_done(data_fixture):
    ...
    SearchHandlerCompat().update_tsvector_columns(table, True)
