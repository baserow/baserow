...
def test_search_workspace_table(data_fixture):
    ...
    # We need to re-migrate tables, to include created rows.
    for t in [table, *context["tables"].values()]:
        t.search_data_state = None
        t.save()
        SearchHandler.migrate_table_tsvectors(t)
    ...
    assert len(search_table.objects.all())


@pytest.mark.django_db
def test_search_builder(data_fixture):
    user = data_fixture.create_user()
    workspace = data_fixture.create_workspace(user=user)
    database = data_fixture.create_database_application(user=user, workspace=workspace)
    table, user, inserted_row, blank_row, context = setup_interesting_test_table(
