...
@pytest.mark.django_db
def test_graph_handler_get_parent_map_empty_graph():
    workflow = MagicMock()
    workflow.graph = {}

    graph_handler = NodeGraphHandler(workflow)

    assert graph_handler.get_parent_map() == {}


@pytest.mark.django_db
def test_graph_handler_get_parent_map_linear_workflow():
    """
    A workflow with no containers maps every node to None (top-level).
    """
    ...
    assert graph_handler.get_parent_map() == {1: None, 2: None, 3: None}


@pytest.mark.django_db
def test_graph_handler_get_parent_map_with_containers():
    workflow = MagicMock()
    workflow.graph = {
        "0": 1,
        "1": {"next": {"": [2]}},
...
@pytest.mark.django_db
def test_graph_handler_get_parent_map_nested_containers():
    workflow = MagicMock()
    workflow.graph = {
        "0": 1,
        "1": {"children": [2]},
