...
def test_get_node_histories_returns_roots_when_no_parent(api_client, data_fixture):
    ...
    assert row["has_error_descendant"] is False
    assert row["edge_label"] == ""
    assert row["status"] == "success"


@pytest.mark.django_db
def test_get_node_histories_returns_children_of_parent(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workflow = data_fixture.create_automation_workflow(user=user)
    ...
    response = api_client.get(
        url + f"?parent_node_id={iterator.id}", **get_api_kwargs(token)
    )

    assert response.status_code == HTTP_200_OK
    rows = response.json()
    assert len(rows) == 1
    assert rows[0]["id"] == child_history.id
    assert rows[0]["parent_node_id"] == iterator.id


@pytest.mark.django_db
def test_get_node_histories_filters_by_iteration_path(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workflow = data_fixture.create_automation_workflow(user=user)
    iterator = data_fixture.create_core_iterator_action_node(workflow=workflow)
    child_in_iterator = data_fixture.create_automation_node(
    ...
    data_fixture.create_automation_node_result(
        node_history=history_iter_0, iteration_path="0"
    )
    history_iter_1 = data_fixture.create_automation_node_history(
        workflow_history=workflow_history, node=child_in_iterator
    )
    data_fixture.create_automation_node_result(
        node_history=history_iter_1, iteration_path="1"
    )
    ...
    response = api_client.get(
        url + f"?parent_node_id={iterator.id}&iteration_path=0",
        **get_api_kwargs(token),
    )

    assert response.status_code == HTTP_200_OK
    rows = response.json()
    assert [r["id"] for r in rows] == [history_iter_0.id]
...
@pytest.mark.django_db
def test_get_node_histories_has_error_descendant(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workflow = data_fixture.create_automation_workflow(user=user)
    iterator = data_fixture.create_core_iterator_action_node(workflow=workflow)
    child_in_iterator = data_fixture.create_automation_node(
    ...
    assert response.status_code == HTTP_200_OK
    rows = {row["id"]: row for row in response.json()}
    iterator_row = next(r for r in rows.values() if r["node"] == iterator.id)
    assert iterator_row["has_error_descendant"] is True
...
def test_get_node_histories_permission_error(api_client, data_fixture):
    ...
    response = api_client.get(url, **get_api_kwargs(other_token))
...
@pytest.mark.django_db
def test_get_node_histories_workflow_history_doesnt_exist(api_client, data_fixture):
...
@pytest.mark.django_db
def test_get_node_result_node_history_doesnt_exist(api_client, data_fixture):
    _, token = data_fixture.create_user_and_token()
...
@pytest.mark.django_db
def test_get_node_result_result_doesnt_exist(api_client, data_fixture):
    """
    Node history exists but has no result row (e.g. errored before write).
    The view surfaces the dedicated 404 error code so the frontend can
    distinguish from a missing node history.
    """
