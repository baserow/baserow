...
class UserFile(models.Model):
    ...
    def deconstruct_name(name):
        ...
        This method expects file names in the format generated by Baserow's upload system:
        'unique_hash.extension' where:
        - unique: alphanumeric string (typically 32 characters)
        - hash: alphanumeric SHA256 hash (64 characters)
        - extension: file extension (can contain special characters)
        
        :param name: The model generated file name.
        :type name: str
        :return: The field name and extracted value mapping.
        :rtype: dict
        :raises InvalidUserFileNameError: If the name is not in the correct format
        """
        ...
        if not matches:
            raise InvalidUserFileNameError(
                name,
                f"The file name '{name}' is not in the correct format. "
                f"Expected format: 'unique_hash.extension' where unique and hash are alphanumeric. "
                f"This usually means you're trying to use an original filename instead of "
                f"the generated name returned by the upload endpoint."
            )
