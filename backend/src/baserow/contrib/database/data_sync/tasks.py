...
def call_strategy_method(method_name, **kwargs):
...
def two_way_sync_row_created(self, **kwargs):
    call_strategy_method("rows_created", **kwargs)


@app.task(bind=True, queue="export")
def two_way_sync_row_updated(self, **kwargs):
    call_strategy_method("rows_updated", **kwargs)


@app.task(bind=True, queue="export")
def two_way_sync_row_deleted(self, **kwargs):
    call_strategy_method("rows_deleted", **kwargs)
