...
from .utils import _build_field_kwargs_for_options, decode_and_validate_edit_token
...
class SubmitFormViewView(APIView):
    ...
    def post(self, request: Request, slug: str) -> Response:
        ...
        field_kwargs = _build_field_kwargs_for_options(
...
class EditRowFormViewView(APIView):
    ...
    def get(self, request: Request, slug: str) -> Response:
        ...
        token = request.query_params.get("edit_token", "")
        row_id, _ = decode_and_validate_edit_token(form, token)
    ...
    @extend_schema(
        ...
        description=(
            ...
            "The `edit_token` must be a valid signed token generated by a "
    ...
    def patch(self, request: Request, slug: str) -> Response:
        ...
        token = data.pop("edit_token", "")
        row_id, _ = decode_and_validate_edit_token(form, token)

        model = form.table.get_model()

        if not model.objects.filter(id=row_id).exists():
            raise RowDoesNotExist(row_id)

        options = form.active_field_options
        field_kwargs = _build_field_kwargs_for_options(model, options)
