...
def notify_admins_in_workspace(
    webhook: TableWebhook, notification_type: NotificationType, data: dict
) -> List[NotificationRecipient]:
    """
    Creates a notification for each admin in the workspace that the webhook belongs to
    informing them that the webhook needs attention. It could be used when a webhook
    fails too many times and is deactivated or if the payload is too large to be sent
    and reach the limit of batches.
...
class WebhookDeactivatedNotificationType(EmailNotificationTypeMixin, NotificationType):
    ...
    def notify_admins_in_workspace(
        ...
        return notify_admins_in_workspace(
            webhook, cls, asdict(DeactivatedWebhookData.from_webhook(webhook))
        )
...
class WebhookPayloadTooLargeNotificationType(
    ...
    def notify_admins_in_workspace(
        ...
        return notify_admins_in_workspace(
            webhook, cls, asdict(WebhookPayloadTooLargeData.from_webhook(webhook))
        )
