...
class FieldDependencyHandler:
    ...
    def _get_all_dependent_fields(
        ...
        fields_db_name = Field._meta.db_table
        tables_db_name = Table._meta.db_table
        ...
        if database_id_prefilter:
            ...
            database_prefilter_query = f"""
                INNER JOIN {fields_db_name}
                    ON ( {relationship_table}.dependant_id = {fields_db_name}.id )
                INNER JOIN {tables_db_name}
                    ON ( {fields_db_name}.table_id = {tables_db_name}.id )
                WHERE  {tables_db_name}.database_id = %(database_id)s
            """
        else:
        ...
        raw_query = f"""
            WITH RECURSIVE traverse(id, dependency_ids, via_ids, depth) AS (
              WITH relationship_table_cte AS ( {relationship_table_cte} )
                ...
                LEFT OUTER JOIN {fields_db_name} as dependant
                    ON first.dependant_id = dependant.id
                LEFT OUTER JOIN {fields_db_name} as dependency
            ...
            SELECT
                ...
                MAX(depth) as depth
            FROM traverse
            LEFT OUTER JOIN {fields_db_name} as field
