...
def log_env_warnings():
    ...
    if settings.BASEROW_PUBLIC_URL:
        if settings.BASEROW_PUBLIC_URL.startswith("http://localhost"):
            logger.warning(
                "WARNING: TCR Projects is configured to use a BASEROW_PUBLIC_URL of "
                f"{settings.BASEROW_PUBLIC_URL}. If you attempt to access TCR Projects on "
                "any other hostname requests to the backend will fail as they will be "
                "from an unknown host. "
                "Please set BASEROW_PUBLIC_URL if you will be accessing TCR Projects "
                f"from any other URL then {settings.BASEROW_PUBLIC_URL}."
            )
    else:
        if "PUBLIC_BACKEND_URL" not in os.environ:
            logger.warning(
                "WARNING: TCR Projects is configured to use a PUBLIC_BACKEND_URL of "
                f"{settings.PUBLIC_BACKEND_URL}. If you attempt to access TCR Projects on any other "
                ...
                f"TCR Projects from any other URL then {settings.PUBLIC_BACKEND_URL}."
            )
        if "PUBLIC_WEB_FRONTEND_URL" not in os.environ:
            logger.warning(
                "WARNING: TCR Projects is configured to use a default PUBLIC_WEB_FRONTEND_URL "
                f"of {settings.PUBLIC_WEB_FRONTEND_URL}. Emails sent by TCR Projects will use links "
                ...
                "the URL where users can access your TCR Projects server."
            )
