...
def getenv(name: str, default: str = "") -> str:
    """
    Slightly more convenient getenv function that return the default value if the
    environment variable is an empty string. This can avoid silly mistakes where an
    a value is picked up from `os.getenv("SOME_VAR", "default") and an empty string
    is used instead because the environment variable is set to an empty string.
    This function is also patched in `test.py` to avoid loading environment variables
    from the `.env` file during tests.
    """
...
FEATURE_FLAGS = [
    flag.strip().lower() for flag in getenv("FEATURE_FLAGS", "").split(",")
]
...
BASEROW_OSS_ONLY = bool(getenv("BASEROW_OSS_ONLY", ""))
if BASEROW_OSS_ONLY:
...
DEBUG = getenv("BASEROW_BACKEND_DEBUG", "off") == "on"

ALLOWED_HOSTS = ["localhost", "127.0.0.1"]
ALLOWED_HOSTS += getenv("BASEROW_EXTRA_ALLOWED_HOSTS", "").split(",")
...
ADDITIONAL_APPS = getenv("ADDITIONAL_APPS", "").split(",")
...
ASGI_HTTP_MAX_CONCURRENCY = (
    int(getenv("BASEROW_ASGI_HTTP_MAX_CONCURRENCY") or 0) or None
)

REDIS_HOST = getenv("REDIS_HOST", "redis")
REDIS_PORT = getenv("REDIS_PORT", "6379")
REDIS_USERNAME = getenv("REDIS_USER", "")
REDIS_PASSWORD = getenv("REDIS_PASSWORD", "")
REDIS_PROTOCOL = getenv("REDIS_PROTOCOL", "redis")
REDIS_URL = getenv(
    "REDIS_URL",
    f"{REDIS_PROTOCOL}://{REDIS_USERNAME}:{REDIS_PASSWORD}@{REDIS_HOST}:{REDIS_PORT}/0",
)

BASEROW_GROUP_STORAGE_USAGE_QUEUE = getenv(
    "BASEROW_GROUP_STORAGE_USAGE_QUEUE", "export"
)
BASEROW_ROLE_USAGE_QUEUE = getenv("BASEROW_GROUP_STORAGE_USAGE_QUEUE", "export")
...
# owns.
CELERY_BEAT_MAX_LOOP_INTERVAL = getenv("CELERY_BEAT_MAX_LOOP_INTERVAL", 20)
...
CELERY_REDBEAT_LOCK_TIMEOUT = getenv(
...
if DATABASE_URL:
    ...
    DATABASES = {
        "default": {
            ...
            "NAME": getenv("DATABASE_NAME", "baserow"),
            "USER": getenv("DATABASE_USER", "baserow"),
            "PASSWORD": getenv("DATABASE_PASSWORD", "baserow"),
            "HOST": getenv("DATABASE_HOST", "db"),
            "PORT": getenv("DATABASE_PORT", "5432"),
        }
    }
    if "DATABASE_OPTIONS" in os.environ:
        DATABASES["default"]["OPTIONS"] = json.loads(getenv("DATABASE_OPTIONS", "{}"))
...
CACHALOT_TIMEOUT = int(getenv("BASEROW_CACHALOT_TIMEOUT", 60 * 60 * 24 * 7))
BASEROW_CACHALOT_ONLY_CACHABLE_TABLES = getenv(
    "BASEROW_CACHALOT_ONLY_CACHABLE_TABLES", None
)
BASEROW_CACHALOT_MODE = getenv("BASEROW_CACHALOT_MODE", "default")
...
BASEROW_CACHALOT_UNCACHABLE_TABLES = getenv("BASEROW_CACHALOT_UNCACHABLE_TABLES", None)
...
CACHALOT_ENABLED = getenv("BASEROW_CACHALOT_ENABLED", "false") == "true"
...
BUILDER_PUBLICLY_USED_PROPERTIES_CACHE_TTL_SECONDS = int(
    ...
    getenv("BASEROW_BUILDER_PUBLICLY_USED_PROPERTIES_CACHE_TTL_SECONDS")
    or 600
)
...
AUTO_INDEX_VIEW_ENABLED = getenv("BASEROW_AUTO_INDEX_VIEW_ENABLED", "true") == "true"
AUTO_INDEX_LOCK_EXPIRY = getenv("BASEROW_AUTO_INDEX_LOCK_EXPIRY", 60 * 2)
...
BASEROW_MAX_CONCURRENT_USER_REQUESTS = int(
    getenv("BASEROW_MAX_CONCURRENT_USER_REQUESTS", "") or -1
)
...
BASEROW_CONCURRENT_USER_REQUESTS_THROTTLE_TIMEOUT = int(
    getenv("BASEROW_CONCURRENT_USER_REQUESTS_THROTTLE_TIMEOUT", 30)
)
...
ACCESS_TOKEN_LIFETIME = timedelta(
    minutes=int(getenv("BASEROW_ACCESS_TOKEN_LIFETIME_MINUTES", 10))  # 10 minutes
)
REFRESH_TOKEN_LIFETIME = timedelta(
    hours=int(getenv("BASEROW_REFRESH_TOKEN_LIFETIME_HOURS", 24 * 7))  # 7 days
)
...
SIMPLE_JWT = {
    ...
    "SIGNING_KEY": getenv("BASEROW_JWT_SIGNING_KEY") or getenv("SECRET_KEY"),
...
BASEROW_FILE_UPLOAD_SIZE_LIMIT_MB = int(
    Decimal(getenv("BASEROW_FILE_UPLOAD_SIZE_LIMIT_MB", 1024 * 1024)) * 1024 * 1024
)  # ~1TB by default

BASEROW_OPENAI_UPLOADED_FILE_SIZE_LIMIT_MB = int(
    getenv("BASEROW_OPENAI_UPLOADED_FILE_SIZE_LIMIT_MB", 512)
)
...
AWS_STORAGE_ENABLED = getenv("AWS_STORAGE_BUCKET_NAME", "") != ""
GOOGLE_STORAGE_ENABLED = getenv("GS_BUCKET_NAME", "") != ""
AZURE_STORAGE_ENABLED = getenv("AZURE_ACCOUNT_NAME", "") != ""
...
BASEROW_PUBLIC_URL = getenv("BASEROW_PUBLIC_URL")
if BASEROW_PUBLIC_URL:
    ...
    PUBLIC_BACKEND_URL = getenv("PUBLIC_BACKEND_URL", "http://localhost:8000")
    PUBLIC_WEB_FRONTEND_URL = getenv("PUBLIC_WEB_FRONTEND_URL", "http://localhost:3000")
...
BASEROW_EMBEDDED_SHARE_URL = getenv("BASEROW_EMBEDDED_SHARE_URL")
if not BASEROW_EMBEDDED_SHARE_URL:
    BASEROW_EMBEDDED_SHARE_URL = PUBLIC_WEB_FRONTEND_URL

PRIVATE_BACKEND_URL = getenv("PRIVATE_BACKEND_URL", "http://backend:8000")
...
FROM_EMAIL = getenv("FROM_EMAIL", "no-reply@localhost")
RESET_PASSWORD_TOKEN_MAX_AGE = 60 * 60 * 48  # 48 hours

ROW_PAGE_SIZE_LIMIT = int(getenv("BASEROW_ROW_PAGE_SIZE_LIMIT", 200))
BATCH_ROWS_SIZE_LIMIT = int(
    getenv("BATCH_ROWS_SIZE_LIMIT", 200)
...
BASEROW_UNIQUE_ROW_VALUES_SIZE_LIMIT = int(
    getenv("BASEROW_UNIQUE_ROW_VALUES_SIZE_LIMIT", 100)
)

# The amount of rows that can be imported when creating a table or data sync.
INITIAL_TABLE_DATA_LIMIT = getenv("INITIAL_TABLE_DATA_LIMIT")
if INITIAL_TABLE_DATA_LIMIT:
    INITIAL_TABLE_DATA_LIMIT = int(INITIAL_TABLE_DATA_LIMIT)

BASEROW_INITIAL_CREATE_SYNC_TABLE_DATA_LIMIT = int(
    getenv("BASEROW_INITIAL_CREATE_SYNC_TABLE_DATA_LIMIT", 5000)
)

MEDIA_URL_PATH = "/media/"
MEDIA_URL = getenv("MEDIA_URL", urljoin(PUBLIC_BACKEND_URL, MEDIA_URL_PATH))
MEDIA_ROOT = getenv("MEDIA_ROOT", "/baserow/media")
...
STALE_MENTIONS_CLEANUP_INTERVAL_MINUTES = int(
    getenv("BASEROW_STALE_MENTIONS_CLEANUP_INTERVAL_MINUTES", "") or 360
)
...
if getenv("EMAIL_SMTP", ""):
    CELERY_EMAIL_BACKEND = "django.core.mail.backends.smtp.EmailBackend"
    # EMAIL_SMTP_USE_TLS OR EMAIL_SMTP_USE_TLS for backwards compatibility after
    # fixing #448.
    EMAIL_USE_TLS = bool(getenv("EMAIL_SMTP_USE_TLS", "")) or bool(
        getenv("EMAIL_SMPT_USE_TLS", "")
    )
    EMAIL_HOST = getenv("EMAIL_SMTP_HOST", "localhost")
    EMAIL_PORT = getenv("EMAIL_SMTP_PORT", "25")
    EMAIL_HOST_USER = getenv("EMAIL_SMTP_USER", "")
    EMAIL_HOST_PASSWORD = getenv("EMAIL_SMTP_PASSWORD", "")

    EMAIL_USE_SSL = bool(getenv("EMAIL_SMTP_USE_SSL", ""))
    ...
    EMAIL_SSL_CERTFILE = getenv("EMAIL_SMTP_SSL_CERTFILE_PATH", None)
    EMAIL_SSL_KEYFILE = getenv("EMAIL_SMTP_SSL_KEYFILE_PATH", None)
else:
...
EMAIL_NOTIFICATIONS_ENABLED = str_to_bool(
    getenv("BASEROW_EMAIL_NOTIFICATIONS_ENABLED", "true")
)
...
EMAIL_NOTIFICATIONS_LIMIT_PER_TASK = {
    "instant": int(getenv("BASEROW_EMAIL_NOTIFICATIONS_LIMIT_INSTANT", 50)),
    "daily": int(getenv("BASEROW_EMAIL_NOTIFICATIONS_LIMIT_DAILY", 1000)),
    "weekly": int(getenv("BASEROW_EMAIL_NOTIFICATIONS_LIMIT_WEEKLY", 5000)),
}
...
EMAIL_NOTIFICATIONS_DAILY_HOUR_OF_DAY = int(
    getenv("BASEROW_EMAIL_NOTIFICATIONS_DAILY_HOUR_OF_DAY", 0)
)
...
EMAIL_NOTIFICATIONS_WEEKLY_DAY_OF_WEEK = int(
    getenv("BASEROW_EMAIL_NOTIFICATIONS_WEEKLY_DAY_OF_WEEK", 0)
)
...
EMAIL_NOTIFICATIONS_AUTO_RETRY_IF_LIMIT_REACHED_AFTER = (
    int(getenv("BASEROW_EMAIL_NOTIFICATIONS_AUTO_RETRY_IF_LIMIT_REACHED_AFTER", 0))
    or None
)
...
MAX_NOTIFICATIONS_LISTED_PER_EMAIL = int(
    getenv("BASEROW_MAX_NOTIFICATIONS_LISTED_PER_EMAIL", 10)
)

# Look into `CeleryEmailBackend` for more information about these settings.
CELERY_EMAIL_CHUNK_SIZE = int(getenv("CELERY_EMAIL_CHUNK_SIZE", 10))
# Use a multiple of CELERY_EMAIL_CHUNK_SIZE to have a sensible rate limit.
MAX_EMAILS_PER_MINUTE = int(getenv("BASEROW_MAX_EMAILS_PER_MINUTE", 50))
...
BASEROW_SEND_VERIFY_EMAIL_RATE_LIMIT = RateLimit.from_string(
    getenv("BASEROW_SEND_VERIFY_EMAIL_RATE_LIMIT", "5/h")
)

login_action_limit_from_env = getenv("BASEROW_LOGIN_ACTION_LOG_LIMIT")
...
BASEROW_SYNC_TEMPLATES_PATTERN = getenv("BASEROW_SYNC_TEMPLATES_PATTERN", None)

MAX_FIELD_LIMIT = int(getenv("BASEROW_MAX_FIELD_LIMIT", 600))
...
BASEROW_ICAL_VIEW_MAX_EVENTS = try_int(
    getenv("BASEROW_ICAL_VIEW_MAX_EVENTS", None), None
)
...
HOURS_UNTIL_TRASH_PERMANENTLY_DELETED = int(
    getenv("HOURS_UNTIL_TRASH_PERMANENTLY_DELETED", 24 * 3)
)
...
DONT_UPDATE_FORMULAS_AFTER_MIGRATION = bool(
    getenv("DONT_UPDATE_FORMULAS_AFTER_MIGRATION", "")
)
EVERY_TEN_MINUTES = "*/10 * * * *"
PERIODIC_FIELD_UPDATE_TIMEOUT_MINUTES = int(
    getenv("BASEROW_PERIODIC_FIELD_UPDATE_TIMEOUT_MINUTES", 9)
)
...
BASEROW_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN = int(
    getenv("BASEROW_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN", 60)
)
PERIODIC_FIELD_UPDATE_QUEUE_NAME = getenv(
    "BASEROW_PERIODIC_FIELD_UPDATE_QUEUE_NAME", "export"
)

BASEROW_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES = int(
    getenv("BASEROW_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES", 8)
)
BASEROW_WEBHOOKS_MAX_RETRIES_PER_CALL = int(
    getenv("BASEROW_WEBHOOKS_MAX_RETRIES_PER_CALL", 8)
)
BASEROW_WEBHOOKS_MAX_PER_TABLE = int(getenv("BASEROW_WEBHOOKS_MAX_PER_TABLE", 20))
BASEROW_WEBHOOKS_MAX_CALL_LOG_ENTRIES = int(
    getenv("BASEROW_WEBHOOKS_MAX_CALL_LOG_ENTRIES", 10)
)
BASEROW_WEBHOOKS_REQUEST_TIMEOUT_SECONDS = int(
    getenv("BASEROW_WEBHOOKS_REQUEST_TIMEOUT_SECONDS", 5)
)
BASEROW_WEBHOOKS_ALLOW_PRIVATE_ADDRESS = bool(
    getenv("BASEROW_WEBHOOKS_ALLOW_PRIVATE_ADDRESS", False)
)
BASEROW_WEBHOOKS_IP_BLACKLIST = [
    ip_network(ip.strip())
    for ip in getenv("BASEROW_WEBHOOKS_IP_BLACKLIST", "").split(",")
    if ip.strip() != ""
]
BASEROW_WEBHOOKS_IP_WHITELIST = [
    ip_network(ip.strip())
    for ip in getenv("BASEROW_WEBHOOKS_IP_WHITELIST", "").split(",")
    if ip.strip() != ""
]
BASEROW_WEBHOOKS_URL_REGEX_BLACKLIST = [
    re.compile(url_regex.strip())
    for url_regex in getenv("BASEROW_WEBHOOKS_URL_REGEX_BLACKLIST", "").split(",")
    if url_regex.strip() != ""
]
BASEROW_WEBHOOKS_URL_CHECK_TIMEOUT_SECS = int(
    getenv("BASEROW_WEBHOOKS_URL_CHECK_TIMEOUT_SECS", "10")
)
BASEROW_MAX_WEBHOOK_CALLS_IN_QUEUE_PER_WEBHOOK = (
    int(getenv("BASEROW_MAX_WEBHOOK_CALLS_IN_QUEUE_PER_WEBHOOK", "0")) or None
)
...
if bool(getenv("BASEROW_ENABLE_SECURE_PROXY_SSL_HEADER", False)):
    SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https")

DISABLE_ANONYMOUS_PUBLIC_VIEW_WS_CONNECTIONS = bool(
    getenv("DISABLE_ANONYMOUS_PUBLIC_VIEW_WS_CONNECTIONS", "")
)

BASEROW_BACKEND_LOG_LEVEL = getenv("BASEROW_BACKEND_LOG_LEVEL", "INFO")
BASEROW_BACKEND_DATABASE_LOG_LEVEL = getenv(
    "BASEROW_BACKEND_DATABASE_LOG_LEVEL", "ERROR"
)

BASEROW_JOB_EXPIRATION_TIME_LIMIT = int(
    getenv("BASEROW_JOB_EXPIRATION_TIME_LIMIT", 30 * 24 * 60)  # 30 days
)
BASEROW_JOB_SOFT_TIME_LIMIT = int(
    getenv("BASEROW_JOB_SOFT_TIME_LIMIT", 60 * 30)  # 30 minutes
)
BASEROW_JOB_CLEANUP_INTERVAL_MINUTES = int(
    getenv("BASEROW_JOB_CLEANUP_INTERVAL_MINUTES", 5)  # 5 minutes
)
BASEROW_ROW_HISTORY_CLEANUP_INTERVAL_MINUTES = int(
    getenv("BASEROW_ROW_HISTORY_CLEANUP_INTERVAL_MINUTES", 30)  # 30 minutes
)
BASEROW_ROW_HISTORY_RETENTION_DAYS = int(
    getenv("BASEROW_ROW_HISTORY_RETENTION_DAYS", 180)
)
BASEROW_MAX_ROW_REPORT_ERROR_COUNT = int(
    getenv("BASEROW_MAX_ROW_REPORT_ERROR_COUNT", 30)
)
BASEROW_MAX_SNAPSHOTS_PER_GROUP = int(getenv("BASEROW_MAX_SNAPSHOTS_PER_GROUP", 50))
BASEROW_SNAPSHOT_EXPIRATION_TIME_DAYS = int(
    getenv("BASEROW_SNAPSHOT_EXPIRATION_TIME_DAYS", 360)  # 360 days
)
BASEROW_USER_LOG_ENTRY_CLEANUP_INTERVAL_MINUTES = int(
    getenv("BASEROW_USER_LOG_ENTRY_CLEANUP_INTERVAL_MINUTES", 60)  # 60 minutes
)
# 61 days to accommodate timezone changes in admin dashboard
BASEROW_USER_LOG_ENTRY_RETENTION_DAYS = int(
    getenv("BASEROW_USER_LOG_ENTRY_RETENTION_DAYS", 61)
)
...
BASEROW_MAX_PENDING_WORKSPACE_INVITES = int(
    getenv("BASEROW_MAX_PENDING_WORKSPACE_INVITES", 0)
)

BASEROW_IMPORT_EXPORT_RESOURCE_CLEANUP_INTERVAL_MINUTES = int(
    getenv("BASEROW_IMPORT_EXPORT_RESOURCE_CLEANUP_INTERVAL_MINUTES", 5)
)
BASEROW_IMPORT_EXPORT_RESOURCE_REMOVAL_AFTER_DAYS = int(
    getenv("BASEROW_IMPORT_EXPORT_RESOURCE_REMOVAL_AFTER_DAYS", 5)
)
...
BASEROW_IMPORT_EXPORT_TABLE_ROWS_COUNT_LIMIT = int(
    getenv("BASEROW_IMPORT_EXPORT_TABLE_ROWS_COUNT_LIMIT", 0)
)
...
OLD_ACTION_CLEANUP_INTERVAL_MINUTES = getenv("OLD_ACTION_CLEANUP_INTERVAL_MINUTES", 5)
MINUTES_UNTIL_ACTION_CLEANED_UP = getenv("MINUTES_UNTIL_ACTION_CLEANED_UP", "120")
...
SYNC_TEMPLATES_ON_STARTUP = getenv("SYNC_TEMPLATES_ON_STARTUP", "true") == "true"
BASEROW_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION = getenv(
...
BASEROW_SYNC_TEMPLATES_TIME_LIMIT = int(
    getenv("BASEROW_SYNC_TEMPLATES_TIME_LIMIT", 60 * 30)
)

APPEND_SLASH = False

BASEROW_DISABLE_MODEL_CACHE = bool(getenv("BASEROW_DISABLE_MODEL_CACHE", ""))
BASEROW_NOWAIT_FOR_LOCKS = not bool(
    getenv("BASEROW_WAIT_INSTEAD_OF_409_CONFLICT_ERROR", False)
)

BASEROW_PERSONAL_VIEW_LOWEST_ROLE_ALLOWED = (
    getenv("BASEROW_PERSONAL_VIEW_LOWEST_ROLE_ALLOWED", "viewer").strip().upper()
)
...
MIGRATION_LOCK_ID = getenv("BASEROW_MIGRATION_LOCK_ID", 123456)
DEFAULT_SEARCH_MODE = getenv("BASEROW_DEFAULT_SEARCH_MODE", "compat")


# Search specific configuration settings.
CELERY_SEARCH_UPDATE_HARD_TIME_LIMIT = int(
    getenv("BASEROW_CELERY_SEARCH_UPDATE_HARD_TIME_LIMIT", 60 * 30)
)
...
USE_PG_FULLTEXT_SEARCH = str_to_bool((getenv("BASEROW_USE_PG_FULLTEXT_SEARCH", "true")))
PG_SEARCH_CONFIG = getenv("BASEROW_PG_SEARCH_CONFIG", "simple")
AUTO_VACUUM_AFTER_SEARCH_UPDATE = str_to_bool(getenv("BASEROW_AUTO_VACUUM", "true"))
TSV_UPDATE_CHUNK_SIZE = int(getenv("BASEROW_TSV_UPDATE_CHUNK_SIZE", "2000"))

POSTHOG_PROJECT_API_KEY = getenv("POSTHOG_PROJECT_API_KEY", "")
...
BASEROW_BUILDER_DOMAINS = getenv("BASEROW_BUILDER_DOMAINS", None)
...
SENTRY_DSN = SENTRY_BACKEND_DSN or getenv("SENTRY_DSN")
...
BASEROW_OPENAI_API_KEY = getenv("BASEROW_OPENAI_API_KEY", None)
BASEROW_OPENAI_ORGANIZATION = getenv("BASEROW_OPENAI_ORGANIZATION", "") or None
BASEROW_OPENAI_MODELS = getenv("BASEROW_OPENAI_MODELS", "")
BASEROW_OPENAI_MODELS = (
    BASEROW_OPENAI_MODELS.split(",") if BASEROW_OPENAI_MODELS else []
)

BASEROW_OPENROUTER_API_KEY = getenv("BASEROW_OPENROUTER_API_KEY", None)
BASEROW_OPENROUTER_ORGANIZATION = (
    getenv("BASEROW_OPENROUTER_ORGANIZATION", "") or None
)
BASEROW_OPENROUTER_MODELS = getenv("BASEROW_OPENROUTER_MODELS", "")
BASEROW_OPENROUTER_MODELS = (
    BASEROW_OPENROUTER_MODELS.split(",") if BASEROW_OPENROUTER_MODELS else []
)

BASEROW_ANTHROPIC_API_KEY = getenv("BASEROW_ANTHROPIC_API_KEY", None)
BASEROW_ANTHROPIC_MODELS = getenv("BASEROW_ANTHROPIC_MODELS", "")
BASEROW_ANTHROPIC_MODELS = (
    BASEROW_ANTHROPIC_MODELS.split(",") if BASEROW_ANTHROPIC_MODELS else []
)

BASEROW_MISTRAL_API_KEY = getenv("BASEROW_MISTRAL_API_KEY", None)
BASEROW_MISTRAL_MODELS = getenv("BASEROW_MISTRAL_MODELS", "")
BASEROW_MISTRAL_MODELS = (
    BASEROW_MISTRAL_MODELS.split(",") if BASEROW_MISTRAL_MODELS else []
)

BASEROW_OLLAMA_HOST = getenv("BASEROW_OLLAMA_HOST", None)
BASEROW_OLLAMA_MODELS = getenv("BASEROW_OLLAMA_MODELS", "")
...
BASEROW_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE = str_to_bool(
    getenv("BASEROW_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE", "true")
)
BASEROW_POSTGRESQL_DATA_SYNC_BLACKLIST = getenv(
